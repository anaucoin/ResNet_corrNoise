...
def conv3x3(in_planes, out_planes, stride=1):
    """
    3x3 convolution with padding, no bias, kernel 3x3
    """
    return nn.Conv2d(in_planes, out_planes, kernel_size=3, stride=stride, padding=1, bias=False)

class PreAct_Block(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, stride=1, downsample=None, noise_coef=None):
        super(PreAct_Block, self).__init__()
...
class PreAct_ResNet_CF(nn.Module):
    def __init__(self, block, layers, num_classes=10, noise_coef=None):
        super(PreAct_ResNet_CF, self).__init__()
        self.inplanes = 16
...
class Ensemble_PreAct_ResNet_CF(nn.Module):
    def __init__(self, block, layers, num_classes=10, num_ensembles=3, noise_coef=0.0):
        super(Ensemble_PreAct_ResNet_CF, self).__init__()
        self.num_ensembles = num_ensembles
        # for emsemble resnet we should use Noisy Blocks.
        self.ensemble = nn.ModuleList([PreAct_ResNet_CF(block, layers, num_classes=num_classes, noise_coef=noise_coef) for i in range(num_ensembles)])
...
def en_PAresnet20_CF(**kwargs):
    model = Ensemble_PreAct_ResNet_CF(PreAct_Block, [3, 3, 3], **kwargs) # 20
...
def progress_bar(batch_num, leng, test_loss, correct, total):
    print('Loss: %.3f | Acc: %.3f%% (%d/%d)' % (test_loss/(batch_num+1), 100.*correct/total, correct, total))
###############################
...
if __name__ == '__main__':
    ...
    batchsize_test = int(batchsize_test)
    ...
    batchsize_train = int(batchsize_train)
    print('Batch size of the train set: ', batchsize_train)
    train_loader = torch.utils.data.DataLoader(dataset=train_set,
                                               batch_size=batchsize_train,
                                               shuffle=True, **kwargs)

    # establish the basic ensemble resnet
    ens_net = en_PAresnet20_CF(num_ensembles=args.num_ensembles,noise_coef=args.noise_coef).cuda()
    ...
    # net = ens_net normally trained network
    net = AttackPGD(ens_net, config).cuda()
...
#########################################################
        ...
        for batch_idx, (x,target) in enumerate(train_loader):
            ...
            progress_bar(batch_idx, len(train_loader),train_loss, correct,total)
...
#########################################################
        ...
        for batch_idx, (x,target) in enumerate(test_loader):
            ...
            total += target.size(0)
            correct += predicted.eq(target.data).cpu().sum()
            progress_bar(batch_idx, len(test_loader),test_loss, correct,total)
            ...
            if correct > best_count:
                print('Saving model...')
                state = {'net': ens_net, 'acc': acc, 'epoch': epoch}

                torch.save(state, './ckpt_PGD_en5_20.t7')
                best_acc = acc
                best_count = correct
